...
const PatientDashboard = () => {
    ...
    const fetchNearestAppointment = async () => {
      try {
        const response = await fetch(
          `http://localhost:8050/api/appointments/nearestAppointment/${email}`
        );
        const data = await response.json();
        console.log(data.nearestAppointment);
        setNearestAppointment(data.nearestAppointment);
        const formattedDate = data.nearestAppointment
          ? new Date(data.nearestAppointment.date).toLocaleString(undefined, {
              dateStyle: "short",
              timeStyle: "short",
            })
          : "";
  ...
  //       if (data?.nearestAppointment) {
  //         setNearestAppointment(data.nearestAppointment);

  //         // Format date into readable form
  //         const formatted = new Date(data.nearestAppointment.date).toLocaleString(undefined, {
  //           dateStyle: 'short',
  //           timeStyle: 'short',
  //         });
  ...
  //         setFormattedDate('No upcoming appointment');
  //         setAppointmentDoctor('Not available');
  //       }
  //     } catch (error) {
  //       console.error('Error fetching nearest appointment:', error);
  //       setFormattedDate('Error fetching data');
  //       setAppointmentDoctor('Unknown');
  //     }
  //   };
      ...
      <tr key={data.hash}>
        ...
        <td>
          <input
            type="button"
            value="Revoke Access"
            onClick={() => removeAccess(data.hash)}
  ...
  async function removeAccess(dhash) {
    ...
    const data = await (
      await fetch(`http://localhost:8080/ipfs/${cookies.hash}`)
    ).json();
    ...
    const fetchDoctors = async () => {
      try {
        ...
        const approvedDoctors = doc.filter((doctor) =>
          patientData.selectedDoctors.includes(doctor.hash)
        );
        console.log(approvedDoctors);
        const unapprovedDoctors = doc.filter(
          (doctor) => !patientData.selectedDoctors.includes(doctor.hash)
        );
        ...
        <div className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg p-6 rounded-2xl w-1200 max-h-[384px] overflow-y-auto">
          ...
          <div className="table-container">
            <table className="modernTable">
              ...
              <tbody>{showApprovedDoctors()}</tbody>
            </table>
