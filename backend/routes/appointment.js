...
router.get("/detailedEarnings", async (req, res) => {
  try {
    const appointments = await Appointment.find(
      {},
      "patientName patientEmail date paymentAmount"
    );
...
router.get("/nearestAppointment/:patientEmail", async (req, res) => {
  ...
  try {
    ...
    if (appointments.length === 0) {
      return res
        .status(404)
        .json({ message: "No appointments found for the patient" });
    }
    ...
    if (!nearestAppointment) {
      return res
        .status(404)
        .json({ message: "No upcoming appointments found for the patient" });
    }
  ...
  try {
    ...
    if (report.length === 0) {
      return res
        ...
        .json({
          message:
            "No appointments found for this doctor within the specified date range",
        });
    }
...
router.put("/denyAppointment/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: "Appointment request denied successfully",
        appointment,
      });
  } catch (error) {
